...
namespace manifold3_moves
{
                                        ...
                                        Cell_handle const& to_be_moved)
  {
    ...
    {
      if (manifold.set_triangulation().delaunay().flip(to_be_moved, i))
      {
#ifndef NDEBUG
  ...
  {
    auto flipped = false;
    if (manifold.set_triangulation().delaunay().flip(
    ...
    for (auto& bottom : one_three)
    {
          ...
          neighboring_31_index)
      {
        ...
        Vertex_handle v_center =
            manifold.set_triangulation().delaunay().tds().insert_in_facet(
                bottom, *neighboring_31_index);

        // Checks
        std::vector<Cell_handle> incident_cells;
        manifold.set_triangulation().delaunay().tds().incident_cells(
  ...
  [[nodiscard]] inline auto do_62_move(Manifold3& manifold)
  {
    ...
    for (auto const& vertex : vertices)
    {
      ...
      {
        manifold.set_triangulation().delaunay().remove(vertex);
        return manifold;
      }
      // Try next vertex
    }

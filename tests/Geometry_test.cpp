  ...
  GIVEN("A 3-dimensional geometry.")
  {
    ...
    WHEN("It is constructed with a Delaunay triangulation.")
    {
      ...
      THEN("Containers of various simplices are correctly filled.")
      {
        ...
        //              (geometry.get_three_one().size() +
        //              geometry.get_two_two().size() +
        //               geometry.get_one_three().size()));
        //        // Every cell is properly labelled
        //        for (auto const& cell : geometry.get_three_one())
        //        { CHECK(cell->info() ==
        //        static_cast<int>(Cell_type::THREE_ONE)); } for (auto const&
        //        cell : geometry.get_two_two()) { CHECK(cell->info() ==
        //        static_cast<int>(Cell_type::TWO_TWO)); } for (auto const& cell
        //        : geometry.get_one_three()) { CHECK(cell->info() ==
        //        static_cast<int>(Cell_type::ONE_THREE)); }

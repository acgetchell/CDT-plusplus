...
INTERNAL_SYMBOL = re.compile(r"(?:::|\b)detail(?:::|\s)|\bcdt::experimental::")
...
def _parse_document(path: Path) -> _DocumentParser:
...
def _validate_required_features(root: Path, documents: dict[Path, _DocumentParser]) -> list[str]:
    ...
    if stylesheet.is_file() and "prefers-color-scheme: dark" not in stylesheet.read_text(encoding="utf-8"):
        errors.append("doxygen.css is missing automatic dark-mode styling")

    graphviz_outputs = sorted(root.rglob("*.svg"))
    if not any("Generated by graphviz" in item.read_text(encoding="utf-8", errors="ignore") for item in graphviz_outputs):
...
def _validate_css(root: Path) -> list[str]:
    """Validate local assets referenced from generated stylesheets."""
    errors: list[str] = []
    for stylesheet in sorted(root.rglob("*.css")):
        for line_number, line in enumerate(stylesheet.read_text(encoding="utf-8").splitlines(), start=1):
...
def validate(site: Path) -> int:
    ...
    documents = {item.resolve(): _parse_document(item.resolve()) for item in sorted(root.rglob("*.html"))}
    errors = _validate_required_pages(root, documents)
    errors.extend(_validate_required_features(root, documents))
    errors.extend(_validate_documents(root, documents))
    errors.extend(_validate_css(root))

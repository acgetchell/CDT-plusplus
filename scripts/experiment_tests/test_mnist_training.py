...
class _TrainingResult(NamedTuple):
    """Replay-sensitive metrics and model state from one training run."""
...
class MnistTrainingTests(unittest.TestCase):
    ...
    @staticmethod
    def _train_once() -> _TrainingResult:
        ...
        return _TrainingResult(metrics=(training_loss, test_loss, accuracy), weights=model.state_dict())
    ...
    def test_training_is_replayable_on_synthetic_cpu_data(self) -> None:
        ...
        self.assertEqual(first.weights.keys(), second.weights.keys())
        for name in first.weights:
            with self.subTest(parameter=name):
                self.assertTrue(torch.equal(first.weights[name], second.weights[name]))

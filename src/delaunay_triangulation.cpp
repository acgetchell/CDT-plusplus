...
{
  std::cerr
      << "NOTICE: This test requires G++ >= 4.4, and will not be compiled."
      << std::endl;
}
#else
...
int main()
{
  double pointsIn[][7] = {{42.89, 0, 60.55, 30.72, 0, 0, 0},
  ...
  int              i = 0;
  for (std::vector<T::Point>::iterator it = points.begin(); it != points.end();
       ++it)
  {
    if (T::Vertex_handle() != hint) { hint = dt.insert(*it, hint); }
    else
    {
      hint = dt.insert(*it);
    }
